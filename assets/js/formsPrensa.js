document.addEventListener("DOMContentLoaded", () => {
    ...
    // Marcar todos los dropdowns con la clase compartida
    ...
    form.addEventListener('submit', (e) => {
        ...
        const payload = new URLSearchParams({
            ...
            subject:     '3ICEO LATAM 2027 - Solicitud Acreditación Prensa y Medios',
            description: [
                fd.get('temas')       ? `Temas de interés: ${fd.get('temas')}` : '',
                fd.get('comentarios') ? `Comentarios: ${fd.get('comentarios')}` : '',
            ].filter(Boolean).join('\n\n'),

            // ── Campos custom — el equipo de backend completa los IDs ──
            // '00NP500000XXXXXX': fd.get('nombre'),
            // '00NP500000XXXXXX': fd.get('apellidos'),
            // '00NP500000XXXXXX': fd.get('medio'),
            // '00NP500000XXXXXX': fd.get('cargo'),
            // '00NP500000XXXXXX': fd.get('sitio_web'),
            // '00NP500000XXXXXX': fd.get('tipo_colaboracion'),
            // '00NP500000XXXXXX': fd.get('tipo_medio'),
            // '00NP500000XXXXXX': fd.get('dias_cobertura'),
